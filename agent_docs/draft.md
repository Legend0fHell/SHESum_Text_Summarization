...
This paper studies a training-free framework for Vietnamese long-document and multi-document summarization. Training-free does not mean model-free. The framework uses pretrained multilingual embeddings, lightweight Vietnamese NLP, graph clustering, and LLM prompting, but it does not train a task-specific summarization model. The core idea is to separate low-cost source processing from expensive abstractive generation. Rule-based splitter output construction, segment construction, semantic chunking, entity and factual phrase extraction, salience ranking, graph construction, and Leiden clustering are performed before LLM summarization. LLM calls are reserved for community-level abstraction and final merging.

The framework is inspired by Context-aware Hierarchical Merging, especially its Extract-Support setting, where source-derived support passages are provided alongside summaries during later merging steps [@context_aware_hm_2025]. The proposed framework adopts the same factuality-oriented principle but changes the structure around it. Instead of merging fixed groups of chunks, the method constructs a weighted chunk graph and applies Leiden community detection to identify chunk communities. Chunk communities are then summarized with source support selected from original segments. At later hierarchy levels, support evidence remains source-only: a parent node can only select support from the union of support units inherited from its children.

The design also draws on hierarchical segmentation work, which argues that long-document summarization benefits from meaningful structure rather than arbitrary fixed-length chunks [@cothssum_2025]. Our implementation therefore starts from splitter-output and paragraph structure, stores overlapping segment windows to reduce the harm of rule-based splitter over-segmentation, and uses semantic chunking to form approximately 1,000 to 1,500-token chunks. The chunk graph combines three signals: document-relative position, entity or factual phrase overlap, and embedding-based semantic similarity. Leiden community detection then gives chunk communities whose granularity can be controlled through graph weights and resolution.

The paper asks the following research questions:

RQ1. Can graph-guided chunk-community formation improve Vietnamese long-document and multi-document summarization compared with sequential hierarchical summarization under comparable LLM settings?

RQ2. Which source-only salience extractor is most effective for Extract-Support prompting: centroid similarity, PACSUM over all segments, or stride-2 PACSUM?
...
2. A source-only evidence propagation rule for hierarchical summarization, where support evidence at every level remains traceable to original segments.
...
This paper evaluates PACSUM with BGE-M3 embeddings in two variants. E2a uses all segments. E2b uses stride-2 candidate segments, corresponding to centers 2, 4, 6, and so on when counted from one. The stride-2 variant reduces overlap between adjacent candidate windows and reduces pairwise PACSUM cost. We intentionally exclude more complicated protected-candidate logic in the first version to keep the ablation focused.
...
MoA is treated as inspiration and an optional baseline, not as the main framing of this paper [@moa_2026]. The present framework has a different emphasis: fewer LLM calls, explicit source-only evidence propagation, and graph-guided chunk-community formation rather than multi-agent LLM deliberation.
...
→ community-level summaries with source support
...
### 3.2 Preprocessing and Segments

The preprocessing stage applies Unicode normalization and whitespace normalization while preserving paragraph boundaries. Each document is split into paragraphs and sentence-like splitter outputs. For each splitter-output index i, the framework constructs a segment:

```text
g_i = concat(x_{i-1}, x_i, x_{i+1})
```

Boundary segments use the available neighboring splitter outputs. Each segment stores the document ID, splitter-output IDs, splitter-output texts, center index, raw text, and embedding. The implementation keeps all segments even when an extraction variant evaluates only a subset.

### 3.3 Semantic Chunking

Chunks are formed with LangChain Experimental SemanticChunker using BGE-M3 embeddings. The default target chunk range is 1,000 to 1,500 whitespace tokens. A simple token chunker is retained only as an explicit ablation selected by configuration. Each chunk stores its text, source segment IDs, position within the document, embedding, and phrase set. Chunk embeddings are computed from the chunk text, while segment embeddings are computed from segment texts.
...
E1 ranks all segments in a chunk by cosine similarity to the chunk embedding:
...
E2a applies PACSUM to all segments in the chunk. It constructs a segment similarity matrix, applies the reference PACSUM thresholding rule, calculates directional centrality scores, and selects top segments under the same evidence budget. The default hyperparameters follow the reference CLI in `libs/PacSum/code/run.py`: beta = 0, lambda1 = 0, and lambda2 = 1. The adaptation in this paper uses BGE-M3 cosine similarities as edge scores.

E2b applies PACSUM only to stride-2 candidate segments. In practice, this uses segments with centers 2, 4, 6, and so on when counted from one. The purpose is to reduce artificial centrality caused by heavily overlapping adjacent segment windows.

Selected segments are compacted before prompting. Rather than printing every selected overlapping window independently, the system merges overlapping selections by splitter-output ID and emits a splitter-output-ordered source span. This reduces prompt redundancy while retaining source provenance.
...
Leiden community detection is applied to the sparse weighted graph. Each community is treated as a chunk community. Chunks are sorted back into source order before summarization so that clustering controls membership but does not scramble the presentation order. If graph clustering is disabled, each chunk becomes its own chunk community, giving a sequential no-graph baseline.
...
### 3.8 Community-Level Extract-Support Summarization

Each chunk community is summarized with an explicit Extract-Support prompt. The prompt contains ordered source text and compacted source support. The LLM is instructed to preserve entities, factual phrases, numbers, dates, and contrastive claims while avoiding unsupported facts.

If the first clustering level yields multiple community-level summaries, the framework performs a final merge. Crucially, merge support is not generated text. The support pool is built from the union of child source support units and reselected using the same salience variant. Thus, the parent support set satisfies:
...
Primary metrics are ROUGE-1, ROUGE-2, and ROUGE-L between the framework output and gold summaries. Efficiency submetrics are input tokens, output tokens, LLM calls, and runtime seconds. The implementation also records chunk count and community count for diagnostic analysis.

ROUGE calculation uses the standard `rouge-score` package. The CSV records the backend to preserve evaluation provenance.

### 4.5 Reproducibility

All experiments should record dataset name, sample ID, salience method, graph weights, graph usage, chunking method, ROUGE scores, input tokens, output tokens, LLM calls, runtime, chunk count, and community count. Result CSVs are aggregated with `scripts/summarize_results.py`.

Example command for a real LLM run:

```powershell
python scripts/run_graphsum.py --dataset vn_mds --data-root datasets --limit 5 --salience e2b --llm openai_compatible --model <model-name> --base-url http://localhost:8000/v1 --output runs/vn_mds_e2b_real.csv
```
...
Third, token-bounded community splitting is not fully implemented. Leiden can create a community larger than the desired prompt budget, especially in collections dominated by one content cluster.
...
This paper proposes a training-free graph-guided Extract-Support framework for Vietnamese long-document and multi-document summarization. The method combines segment source evidence units, paragraph-aware semantic chunking, lightweight entity and factual phrase extraction, sparse weighted chunk graphs, Leiden chunk-community clustering, and source-only evidence propagation. The framework is designed to reduce LLM calls and token usage while preserving source-grounded support for factual summarization. The next step is to complete real LLM experiments on VN-MDS and ViMs, evaluate English generalization on Multi-News, and run the planned salience and graph-weight ablations.
